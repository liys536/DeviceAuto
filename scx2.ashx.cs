...
namespace DeviceAuto
{
    ...
    /// scx2 的摘要说明
    /// </summary>
    public class scx2 : IHttpHandler
    {
        ...
        private void Query(string bmid)
        {
            try
            {
                ...
                SqlHelper.ExeNonQuery("get_bmzl_scx", CommandType.StoredProcedure, parms);


                DataSet duser = SqlHelper.GetList("v_scx", "*", "id", int.Parse(rows), int.Parse(page), false, false, strWhere);
                DataTable dt1 = duser.Tables[0];
                //获取数据源
                DataTable dt = SqlHelper.GetTable("select *  from v_scx where " + strWhere);
        ...
        private void Del()
        {
            try
            {
                ...
                int count = 0;
                count = SqlHelper.DelData("scxb", id);
                if (count > 0)
                {
        ...
        private void Add()
        {
            try
            {
                ...
                string scx = str[2].Split('=')[1];
                //重要：将数组最后一维后面的'\n'去掉，否则出现tree无法加载的情况
                scx = scx.Substring(0, scx.Length - 1);
                ...
                //判断分类名称是否存在
                DataTable dt = SqlHelper.GetTable("select * from scxb where cscx='" + scx + "'");
                if (dt.Rows.Count > 0)
                {
                ...
                SqlParameter[] parms = {
                            ...
                            new SqlParameter("@scx",scx)
                                 };
                bool flag = SqlHelper.ExeNonQuery("insert_scx", CommandType.StoredProcedure, parms);
                if (flag)
                {
        ...
        private void Edit()
        {
            try
            {
                ...
                //判断分类名称是否存在
                DataTable dt = SqlHelper.GetTable("select * from scxb where cscx='" + scx + "' and id<>" + id);
                if (dt.Rows.Count > 0)
                {
                    HttpContext.Current.Response.Write("3");
                    return;
                ...
                bool flag = SqlHelper.ExeNonQuery("update_scx", CommandType.StoredProcedure, parms);
                if (flag)
                {
