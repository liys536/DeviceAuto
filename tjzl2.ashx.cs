...
namespace DeviceAuto
{
    ...
    /// tjzl2 的摘要说明
    /// </summary>
    public class tjzl2 : IHttpHandler
    {
        ...
        private void Query(string id)
        {
            try
            {
                ...
                SqlHelper.ExeNonQuery("get_tjzl", CommandType.StoredProcedure, parms);

                DataSet duser = SqlHelper.GetList("v_tjzl", "*", "id", int.Parse(rows), int.Parse(page), false, false, strWhere);
                DataTable dt1 = duser.Tables[0];
                //获取数据源
                DataTable dt = SqlHelper.GetTable("select *  from v_tjzl where " + strWhere);
        ...
        private void Del()
        {
            try
            {
                ...
                //判断停机记录中是否存在
                sSql = "select * from tjjlb where ctjzl=" + id;
                ...
                //判断该停机种类是否有下级种类
                sSql = "select * from tjzlb where pid=" + id;
                dt = SqlHelper.GetTable(sSql);
                if (dt.Rows.Count > 0)
                {
                ...
                int count = 0;
                count = SqlHelper.DelData("tjzlb", id);
                if (count > 0)
                {
        ...
        private void Add()
        {
            try
            {
                ...
                string[] str = ss.Split('&');
                string sjzl = str[1].Split('=')[1];
                string tjbh = str[2].Split('=')[1];
                string tjzl = str[3].Split('=')[1];
                string tjms = str[4].Split('=')[1];
                //重要：将数组最后一维后面的'\n'去掉，否则出现tree无法加载的情况
                tjms = tjms.Substring(0, tjms.Length - 1);

                if (sjzl.Trim().Length == 0)
                {
                    HttpContext.Current.Response.Write("1");
                    return;
                }

                if (tjzl.Trim().Length == 0)
                {
                    HttpContext.Current.Response.Write("2");
                    return;
                }
                ...
                //判断分类名称是否存在
                DataTable dt = SqlHelper.GetTable("select * from tjzlb where ctjzl='" + tjzl + "'");
                if (dt.Rows.Count > 0)
                {
                ...
                SqlParameter[] parms = {
                            new SqlParameter("@sjzl",sjzl),
                            new SqlParameter("@tjbh",tjbh),
                            new SqlParameter("@tjzl",tjzl),
                            new SqlParameter("@tjms",tjms)
                                 };
                bool flag = SqlHelper.ExeNonQuery("insert_tjzl", CommandType.StoredProcedure, parms);
                if (flag)
                {
        ...
        private void Edit()
        {
            try
            {
                ...
                int sjzl = int.Parse(str[1].Split('=')[1]);
                string tjbh = str[2].Split('=')[1];
                string tjzl = str[3].Split('=')[1];
                string tjms = str[4].Split('=')[1];
                tjms = tjms.Substring(0, tjms.Length - 1);
                ...
                //判断分类名称是否存在
                DataTable dt = SqlHelper.GetTable("select * from tjzlb where ctjzl='" + tjzl + "' and id<>" + id);
                if (dt.Rows.Count > 0)
                {
                    HttpContext.Current.Response.Write("3");
                    return;
                ...
                bool flag = SqlHelper.ExeNonQuery("update_tjzl", CommandType.StoredProcedure, parms);
                if (flag)
                {
